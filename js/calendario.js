document.addEventListener('DOMContentLoaded', () => {
                ...
                reservas.forEach(r => {
                    ...
                    li.innerHTML = `
                        ...
                        <div class="reserva-actions">
                            <span class="reserva-estado">${r.estado.charAt(0).toUpperCase() + r.estado.slice(1)}</span>
                            ${r.estado === 'activa' ? `<button class="btn-finalizar" data-reserva-id="${r.id_reserva}">Finalizar</button>` : ''}
                        </div>
                    `;
                    listaMisReservas.appendChild(li);
                });

                // Añadir listeners a los nuevos botones de finalizar
    ...
    const finalizarReserva = async (reservaId, button) => {
        ...
        try {
            ...
            if (response.ok) {
                // 1. ¡Paso clave! Actualizamos los datos del usuario en localStorage con la respuesta del backend.
                authManager.updateUserData(data.user);
                // 2. Recargamos la lista de reservas para que muestre el estado "finalizada".
                cargarReservasUsuario();
                // 3. Forzamos la actualización del header para que muestre el nuevo total de tokens.
                actualizarUI();
            }
        } catch (error) {
    ...
    bookingForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (reservaResponse.ok) {
                // ¡Paso clave! Actualizamos los datos del usuario en localStorage con la respuesta del backend.
